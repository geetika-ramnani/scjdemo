...
// Example admin route
router.get('/admin', auth, authoriz('admin'), (req, res) => res.json({ message: 'Admin access granted' }));
...
router.get('/auth/x/callback', passport.authenticate('twitter', { session: false }), (req, res) => {
  const token = jwt.sign({ id: req.user.id, email: req.user.email }, process.env.JWT_SECRET, { expiresIn: '7d' });
  res.redirect(`${process.env.FRONTEND_URL}/oauth-callback?token=${token}`);
});
